...
#[cfg(test)]
mod tests {
    use crate::trackers::WebPageTarget;
    ...
    fn deserialization() -> anyhow::Result<()> {
        ...
        assert_eq!(
            serde_json::from_str::<WebPageTarget>(&json!({ "extractor": "export async function execute(p, r) { await p.goto('https://retrack.dev/'); return r.html(await p.content()); }" }).to_string())?,
            target
        );
